...
import {
  getPostBySlug, getPostBySlugAdmin, getAllPosts, getSeriesPosts,
  getRelatedPosts, getApprovedComments, parseTags,
} from '@/lib/blog'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      type: 'article', publishedTime: post.createdAt, authors: [siteConfig.name],
    },
  }
}
        ...
        <div className="flex-1 min-w-0 max-w-3xl">
          ...
          {isPreview && !post.published && (
            <div className="mb-6 flex items-center gap-3 px-4 py-3 rounded-xl border border-amber-500/30 bg-amber-500/8 text-sm text-amber-400">
              <span className="text-xs font-bold uppercase tracking-widest bg-amber-500/20 border border-amber-500/30 px-2 py-0.5 rounded-full">Draft</span>
