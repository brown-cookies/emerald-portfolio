...
import { Eye, Code2, Save, Loader2, Trash2, Globe, FileText, ExternalLink, CheckCircle, AlertCircle } from 'lucide-react'
...
export default function PostEditor({ post, allSeries }: Props) {
  ...
  const [seriesOrder, setSeriesOrder] = useState<string>(String(post?.seriesOrder ?? ''))
  ...
  useEffect(() => {
    if (!debouncedSlug) { setSlugStatus('idle'); return }
    // Skip check if slug hasn't changed from the saved post
    if (post && debouncedSlug === post.slug) { setSlugStatus('idle'); return }
  ...
  useEffect(() => { setIsDirty(true) }, [title, slug, description, content, tags, seriesId, seriesOrder, published])
  ...
  }, [isDirty, title, slug, description, content, tags, seriesId, seriesOrder, published]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const fetchPreview = useCallback(async (src: string) => {
    if (!src.trim()) { setPreviewHtml(''); return }
  ...
  async function doSave(asDraft = false, silent = false) {
    ...
    if (res.ok) {
      if (!silent) setSuccess(post ? 'Post updated.' : 'Post created.')
      if (!post) startTransition(() => router.push(`/admin/posts/${data.post.id}/edit`))
  ...
  async function handleSave(asDraft = false) { await doSave(asDraft, false) }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button
          ...
          disabled={isPending || !title.trim() || !slug.trim() || slugStatus === 'taken'}
        ...
        {post && (
          <span className="text-xs text-muted-foreground flex items-center gap-1.5">
            {autoSaving ? (
              ...
              <><CheckCircle className="w-3 h-3 text-emerald-500" /> Saved {autoSaveAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</>
            ) : null}
          </span>
        )}
      ...
      <div className="grid grid-cols-1 gap-3">
        ...
        <div className="relative">
          ...
          <span className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 text-xs">
            {slugStatus === 'checking' && <><Loader2 className="w-3 h-3 animate-spin text-muted-foreground" /><span className="text-muted-foreground">Checking…</span></>}
            {slugStatus === 'available' && <><CheckCircle className="w-3 h-3 text-emerald-500" /><span className="text-emerald-500">Available</span></>}
            {slugStatus === 'taken' && <><AlertCircle className="w-3 h-3 text-red-400" /><span className="text-red-400">Taken</span></>}
          </span>
        </div>
        ...
        <input value={description} onChange={e => setDescription(e.target.value)}
          placeholder="Short description (shown in cards and meta)" className={inputCls} />
        <input value={tags} onChange={e => setTags(e.target.value)}
          placeholder="Tags: ai, python, robotics  (comma separated)" className={inputCls} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <input type="number" value={seriesOrder} onChange={e => setSeriesOrder(e.target.value)}
          placeholder="Part # in series (1, 2, 3…)" min={1} className={inputCls} />
      </div>

      {/* ── Content editor — write / preview tabs ── */}
      <div className="rounded-2xl border border-border overflow-hidden">
        <div className="flex border-b border-border bg-muted/40">
          <button onClick={() => handleTabSwitch('write')}
          ...
          <button onClick={() => handleTabSwitch('preview')}
        ...
        {tab === 'write' ? (
          ...
          <div className="min-h-[420px] px-8 py-6 bg-background">
            {previewErr ? (
              ...
              <article className="mdx-content" dangerouslySetInnerHTML={{ __html: previewHtml }} />
            ) : (
              <p className="text-muted-foreground/40 text-sm">Nothing to preview yet.</p>
            )}
          </div>
        )}
      </div>
