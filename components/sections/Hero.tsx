...
import { ArrowRight, Github, Linkedin, Mail, Download, Sparkles, MapPin, User } from 'lucide-react'
...
const ROLES = ['Web Engineer', 'Software Engineer', 'AI Engineer', 'Full-Stack Developer']
...
const photoVariant = {
  ...
  visible: { opacity: 1, scale: 1,    x: 0,  transition: { duration: 0.8, delay: 0.3, ease: [0.21, 0.47, 0.32, 0.98] as [number, number, number, number] } },
}
...
export default function Hero() {
  ...
  return (
    <section className="relative h-[100dvh] flex items-center overflow-hidden bg-background">
      {/* Background layers — particles are global, so we just add beams + spotlight here */}
      <BackgroundBeams className="opacity-30" />
      <Spotlight className="-top-40 -left-20 md:left-40 md:-top-20" fill="#10b981" />
          ...
          <motion.div variants={container} initial="hidden" animate="visible">
            ...
            <motion.div variants={item} className="mb-6">
              <span className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-emerald-500/25 bg-emerald-500/5 text-emerald-500 text-xs font-mono backdrop-blur-sm">
                ...
                {siteConfig.availableForWork ? 'Available for new projects' : 'Open to opportunities'}
              </span>
            </motion.div>

            {/* Name */}
            <motion.div variants={item} className="mb-3">
              <p className="font-body text-muted-foreground/80 text-base mb-1">Hi, I&apos;m</p>
            ...
            {/* Typewriter role */}
            <motion.div variants={item} className="flex items-center gap-2 text-lg sm:text-xl font-display font-semibold text-muted-foreground mb-4 h-8">
              <Sparkles className="w-4 h-4 text-emerald-500 flex-shrink-0" />
              <TypewriterLoop strings={ROLES} className="text-foreground" />
            </motion.div>

            {/* Tagline */}
            <motion.p variants={item} className="text-muted-foreground text-base leading-relaxed mb-2 max-w-lg">
              {siteConfig.tagline}
            </motion.p>

            {/* Location */}
            <motion.div variants={item} className="flex items-center gap-1.5 text-xs font-mono text-muted-foreground mb-8">
            ...
            <motion.div variants={item} className="flex flex-wrap items-center gap-3 mb-8">
              <MagneticButton>
                <Button size="lg" className="gap-2 shadow-lg shadow-emerald-500/25" onClick={() => goTo('projects')}>
                  View My Work <ArrowRight className="w-4 h-4" />
                </Button>
              </MagneticButton>
              <MagneticButton href={siteConfig.resumeUrl} download>
                <MovingBorderButton duration={3000} className="flex items-center gap-2 text-foreground font-medium">
            ...
            <motion.div variants={item} className="flex items-center gap-5">
              {[
                ...
                >
                  ...
                  <span className="text-xs font-mono opacity-0 group-hover:opacity-100 transition-opacity">{label}</span>
                </a>
              ))}
            </motion.div>
          </motion.div>
          ...
          >
            ...
            <div className="relative w-[280px] h-[280px]">
              ...
              <div className="relative w-full h-full rounded-[30%_70%_70%_30%_/_30%_30%_70%_70%] border-2 border-emerald-500/30 overflow-hidden bg-card">
                {imgError ? (
                  /* ── Placeholder shown when profile.jpg is missing ── */
                  <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 gap-3">
                    <div className="w-16 h-16 rounded-full bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center">
                      <User className="w-8 h-8 text-emerald-500/60" />
                    </div>
                    <p className="font-mono text-[9px] text-emerald-500/50 text-center px-4 leading-relaxed">
                      Add your photo at<br />
                      <span className="text-emerald-500/80">public/images/profile.jpg</span>
                    </p>
                  </div>
                ) : (
                  /* ── Real photo — plain <img>, no next/image optimization loop ── */
            ...
            >
              <p className="font-display font-bold text-lg text-emerald-500">4+</p>
              <p className="text-[10px] text-muted-foreground font-mono">Years coding</p>
            </motion.div>
            ...
            >
              <p className="font-display font-bold text-lg text-emerald-500">15+</p>
              <p className="text-[10px] text-muted-foreground font-mono">Projects shipped</p>
            </motion.div>
