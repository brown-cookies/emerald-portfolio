...
import { Award, ExternalLink, Shield, Brain, Wifi, Globe, Cloud } from 'lucide-react'
import { BackgroundGradient } from '@/components/aceternity/background-gradient'
import { Badge } from '@/components/ui/badge'
import { type Certification } from '@/data/config'

const META = {
  web:        { label: 'Web Dev',    icon: Globe,  color: 'text-blue-400',   bg: 'bg-blue-400/10',   badge: 'blue'    as const },
  ai:         { label: 'AI / ML',    icon: Brain,  color: 'text-purple-400', bg: 'bg-purple-400/10', badge: 'purple'  as const },
  security:   { label: 'Security',   icon: Shield, color: 'text-red-400',    bg: 'bg-red-400/10',    badge: 'default' as const },
  networking: { label: 'Networking', icon: Wifi,   color: 'text-amber-400',  bg: 'bg-amber-400/10',  badge: 'amber'   as const },
  cloud:      { label: 'Cloud',      icon: Cloud,  color: 'text-emerald-500',bg: 'bg-emerald-500/10',badge: 'emerald' as const },
}

const FILTERS = ['All', 'web', 'ai', 'security', 'networking', 'cloud'] as const
                ...
                <BackgroundGradient animate={false} containerClassName="h-full" className="h-full">
                  <div className="p-6 flex flex-col gap-4 h-full">
                    <div className="flex items-start justify-between">
                      ...
                      <Badge variant={m.badge as 'blue' | 'purple' | 'default' | 'amber' | 'emerald'}>{m.label}</Badge>
                    </div>
