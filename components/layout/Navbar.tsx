...
import { Menu, X, Award, Download, Sparkles, SparklesIcon } from 'lucide-react'
...
function BgToggle() {
  ...
  return (
    <button
      ...
      aria-label={bgEnabled ? 'Disable animated background' : 'Enable animated background'}
    ...
    <>
      ...
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          {/* Logo */}
          <button onClick={() => handleNav('hero')} className="group flex items-center gap-1.5">
            <span className="font-display font-extrabold text-xl tracking-tight">
              <span className="bg-gradient-to-r from-emerald-400 to-emerald-600 bg-clip-text text-transparent">M</span>
              <span>ielle</span>
            </span>
            <span className="font-mono text-[10px] text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity">.dev</span>
          </button>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div className="fixed inset-0 z-40 md:hidden"
            ...
            <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setMobile(false)} />
            <motion.aside
            ...
            >
              {NAV_LINKS.map(({ label, id }, i) => (
                <motion.div key={id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 + 0.1 }}>
                  <button
                    onClick={() => handleNav(id)}
                    className={`w-full text-left px-4 py-3 text-base font-display font-semibold rounded-xl transition-colors ${
                      current === id ? 'bg-emerald-500/10 text-emerald-500' : 'hover:bg-emerald-500/8 hover:text-emerald-500'
                    }`}
                  >
                    {label}
                  </button>
              ...
              <Link href="/certifications" onClick={() => setMobile(false)}
              ...
              <div className="mt-6 pt-4 border-t border-border space-y-2">
                <Button asChild className="w-full gap-2">
                  <a href={siteConfig.resumeUrl} download><Download className="w-4 h-4" /> Download Resume</a>
                </Button>
              </div>
            </motion.aside>
          </motion.div>
